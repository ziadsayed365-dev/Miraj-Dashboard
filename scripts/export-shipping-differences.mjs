...
// pay". The Orders column is context only (how much volume sits behind the rate).
//
...
// Rolling window: only orders placed (order_created_at) in the last N days.
// The rate figures are per-order and window-independent, but the Orders column
// (volume context) and the modal Our Rate now reflect recent activity only.
...
async function fetchOrders() {
  ...
  for (;;) {
    const batch = await get(
      `orders?select=${encodeURIComponent(select)}&cancelled_at=is.null&order_created_at=gte.${since}&id=gt.${lastId}&order=id.asc&limit=1000`
    );
    rows.push(...batch);
...
wb.created = new Date();
const ws = wb.addWorksheet(`Shipping Rates (last ${WINDOW_DAYS} days)`, { views: [{ state: "frozen", ySplit: 1 }] });
...
const outPath = path.join(process.cwd(), "shipping-differences.xlsx");
await wb.xlsx.writeFile(outPath);

console.log(`Wrote ${rows.length} governorates (orders placed since ${since.slice(0, 10)}, last ${WINDOW_DAYS} days) -> ${outPath}\n`);
const pad = (s, n) => String(s).padStart(n);
console.log(`${"Governorate".padEnd(18)}${pad("Orders", 7)}${pad("Ours", 8)}${pad("Bosta", 8)}${pad("Diff", 9)}${pad("NextDay", 9)}${pad("OpenPkg", 9)}${pad("Total", 9)}${pad("Net", 9)}`);
for (const r of rows) {
  console.log(
