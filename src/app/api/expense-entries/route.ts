...
import { createExpenseEntry } from "@/lib/reports/expenses-ledger";
...
export async function POST(request: NextRequest) {
  ...
  const { date, accountId, amount, note } = body;
  if (typeof date !== "string" || typeof accountId !== "number" || typeof amount !== "number") {
    return NextResponse.json({ ok: false, error: "Invalid input" }, { status: 400 });
  }

  try {
    const id = await createExpenseEntry({ date, accountId, amount, note: typeof note === "string" ? note : null });
